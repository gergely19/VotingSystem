...
namespace VotingSystem.DataAccess.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<(string authToken, string refreshToken, string userId)> LoginAsync(string email, string password)
        {
            ...
            if (user == null)
                throw new UnauthorizedAccessException("Email vagy jelszó helytelen");

            var result = await _signInManager.PasswordSignInAsync(user.Email!, password, false, true);
            if (result.IsLockedOut)
                throw new AccessViolationException("Túl sok sikertelen próbálkozás. A felhasználó zárolva lett.");
